...
class Exercise:
    ...
    def find_choices(self, right, wrong, new_sent): #TODO @Kate, rewrite this function
        ...
        elif self.error_type == ['Incoherent_conj','Concession','Contrast']:
            #переписать с использованием nltk.stanford
            conjunctions1 = ['except', 'besides','but for']
            conjunctions2 = ['even if', 'even though', 'even']
            if (right in conjunctions1) or (wrong in conjunctions1):
                [choices.append(i) for i in conjunctions1 if (i != right) and (i != wrong)]
            elif (right in conjunctions2) or (wrong in conjunctions2):
...
if __name__ == "__main__":
    ...
    e = Exercise(path_to_data, ['Incoherent_conj','Concession','Contrast'], ['multiple_choice'], bold=False, context=True)
