    ...
    def find_choices(self, right, wrong, new_sent): #TODO @Kate, rewrite this function
        ...
        if self.error_type == ['Number']:
            ...
            if verb_forms:
                ...
                verb_forms = vff.find_verb_forms(right.split(' ')[0])
                if verb_forms:
                    verb_right_context = ' '.join(right.split(' ')[1:])
            if verb_forms:
                quant_presence = False
                splitsent = new_sent.split(' ')
                for i in ('some', 'someone', 'somebody', 'one', 'everyone', 'everybody', 'noone', 'no-one', 'nobody', 'something', 'everything', 'nothing'):
                    if (i in splitsent) and (ind(splitsent,i)<ind(splitsent,"_______")):
                        quant_presence = True
                if quant_presence:
                    [choices.append(i) for i in (vff.neg(right), vff.neg(wrong)) if (i!=right) and (i!=wrong) and (i!='') and (i!=None)]
        #смотрим на то, какой массив мы передали объекту, так как отследить, какая ошибка обрабатывается сейчас мы уже не можем
            ...
            if pr1['prep']:
                ...
                if ((left + rite).strip(' ') == (pr2['left']+pr2['right']).strip(' ')):
                    ...
                    print(left+rite,';', pr2['left']+pr2['right'])
            elif pr2['prep']:
                ...
                if left+rite.strip(' ') == pr1['left'].strip(' '):
                    ...
                    print(left+rite,';', pr1['left']+pr1['right'],';', pr1['left'])
        #на союзы(пункт 4-5) не находит примеров
        elif self.error_type == ['Incoherent_conj','Concession']:
            #переписать с использованием nltk.stanford
            print(choices)
            conjuctions1 = ['except', 'besides','but for']
            conjuctions2 = ['even if', 'even though', 'even']
            if right in conjuctions1:
...
if __name__ == "__main__":
    ...
    path_to_data = './IELTS2016/'
    #передаём только определённый вариат задания
    e = Exercise(path_to_data, ['Incoherent_conj','Concession'], ['multiple_choice'], bold=False, context=True)
