...
def find_verb_forms(w):
    ...
    be_forms = {'bare_inf':'be', '2nd':'was', '2ndPL':'were', '3rd':'been', 'gerund':'being', '1SG':'am', 'non_3SG':'are', '3SG':'is'}
    have_forms = {'non_3SG':'have', '2nd':'had', '3rd':'had', '3SG':'has', 'gerund':'having'}
        ...
        for form in all_forms[word]:
            ...
            if match and match.group(0) == form:
                ...
                if (stem not in stems) and not(len(stem)>2 and stem.endswith('ly')):
                    ...
                    for key, variants in another_form_variants.items():
                        for variant in variants:
                            if variant in forms_completed:
                                ...
                                stems.add(stem)
                                non_3SG_form_variants = [stem,stem+'e',stem[:len(stem)-1]+'y',stem[:len(stem)-1]+'ie']
                                past_form_variants = [stem+stem[len(stem)-1]+'ed',stem[:len(stem)-1]+'ied',stem+'ed',stem+'d']
                                for i in non_3SG_form_variants:
                                    if i in forms_completed:
                                        verb_forms[stem].append(i)
                                        verb_forms_marked['non_3SG'] =  i
                    ...
                    if ('non_3SG' in verb_forms_marked) and (verb_forms_marked['non_3SG'] in irregular_forms):
                        verb_forms_marked['2nd'] = irregular_forms[ verb_forms_marked['non_3SG'] ][0]
                        verb_forms_marked['3rd'] = irregular_forms[ verb_forms_marked['non_3SG'] ][1]
...
def neg(verb_form):
    ...
    if (head_verb in dSN) or ((head_verb in tuple(have_forms[i] for i in have_forms)) and (other_verbs)):
        ...
        non_3SG_phrase = ' ' + head_verb_forms['non_3SG'] + ' ' + other_verbs
        if head_verb == head_verb_forms['2nd']:
            return "didn't" + non_3SG_phrase
        elif head_verb == head_verb_forms['3SG']:
            return "doesn't" + non_3SG_phrase
        elif head_verb == head_verb_forms['non_3SG']:
            return "don't" + non_3SG_phrase
    else:
        return ''

def get_analytic_forms(verb):
    synth_forms = find_verb_forms(verb)
    verb = synth_forms['3SG']
    form_attrs = {'Passive': [False, True] 'Continuous': [False, True] 'Perfect': [False, True] 'Future': [False, True] 'Past': [False, True]\
                  'non_3SG': [False, True] '1SG': [False, True] 'Inf': [False, True]}
